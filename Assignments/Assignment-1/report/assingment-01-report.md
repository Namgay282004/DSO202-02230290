...
The lifecycle begins with the **kube-apiserver**, which serves as the central hub. It validates my YAML manifests for the namespace, deployments, and services, persisting the desired state into **etcd**. The **kube-scheduler** then monitors the cluster for "unscheduled" pods. It performs a filtering and scoring process: first filtering out nodes that cannot satisfy the **ResourceQuota** and **LimitRange** defined in Task 6, and then scoring the remaining nodes to find the optimal placement. In a single-node `kind` cluster, the scheduler assigns all workloads to the `kind-control-plane` node.

**The Node Components (Execution Layer):**
Once a pod is scheduled, the **kubelet** on the node takes over. It coordinates with the **Container Runtime (Docker)** via the CRI to pull my custom ARM64 images from Docker Hub. The kubelet ensures that the containers start according to the PodSpec and maintains the health of the pods. Simultaneously, **kube-proxy** manages the virtual networking. It manipulates IPtables or IPVS rules on the node to ensure that internal traffic to the **Headless Service** (Database) and **ClusterIP** (Backend) is routed correctly, and that the **NodePort** (Frontend) is accessible to the host.
...
## 6. Conclusion
This assignment successfully demonstrates the power of Kubernetes as an orchestrator. By implementing persistence, internal service discovery, and resource governance, I have created a resilient environment where the application can self-heal and protect its resources.